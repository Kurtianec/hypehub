  ...
  return (
    <div>
      {products.filter((p) => p.status === "available").length < 5 && (
        <div className="mb-4 rounded-xl border border-[#8E1537]/30 bg-[#8E1537]/5 p-3 text-sm text-[#8E1537]">
      ...
      <div className="flex flex-col sm:flex-row gap-2 mb-4">
        <div className="relative flex-1">
          ...
          <Input
            ...
            className="pl-10 bg-[#0A0A0A] border-2 border-[#2A2A2A] focus:border-[#8E1537] font-mono"
          />
        </div>
        <Select value={statusFilter} onValueChange={setStatusFilter}>
          <SelectTrigger className="w-full sm:w-48 bg-[#0A0A0A] border-2 border-[#2A2A2A] focus:border-[#8E1537] font-mono">
      ...
      {selected.size > 0 && (
        <motion.div
          ...
          className="bg-[#0E0E0E] border-2 border-[#8E1537] p-3 mb-4 flex flex-wrap items-center gap-3"
          style={{ clipPath: "polygon(0 0, calc(100% - 12px) 0, 100% 12px, 100% 100%, 12px 100%, 0 calc(100% - 12px))" }}
        >
          <span className="text-sm font-black text-[#8E1537] font-mono uppercase">
          ...
          <Button
            ...
            disabled={!bulkAction}
            size="sm"
            className="bg-[#8E1537] text-black hover:bg-[#8E1537] hover:text-white font-black uppercase font-mono"
          >
            Применить
          </Button>
          <Button
      ...
      {filtered.length > 0 && (
        <div className="flex items-center gap-3 mb-3 px-1">
          <button
            ...
            className="flex items-center gap-2 text-xs font-mono uppercase text-[#888] hover:text-[#8E1537]"
          >
            {selected.size === filtered.length && filtered.length > 0 ? (
              <CheckSquare className="w-4 h-4 text-[#8E1537]" strokeWidth={2.5} />
            ) : (
            ...
            return (
              <motion.div
                ...
                className={cn(
                  ...
                  isSelected ? "border-[#8E1537]" : "border-[#2A2A2A] hover:border-[#8E1537]/60"
                )}
                style={{ clipPath: "polygon(0 0, calc(100% - 12px) 0, 100% 12px, 100% 100%, 12px 100%, 0 calc(100% - 12px))" }}
              >
                {/* Checkbox + status */}
                <div className="flex items-center justify-between mb-2">
                  ...
                  >
                    {isSelected ? (
                      <CheckSquare className="w-5 h-5 text-[#8E1537]" strokeWidth={2.5} />
                    ) : (
                ...
                <div className="flex gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex flex-wrap gap-1 mb-1">
                      {p.featured && (
                        <span className="px-1.5 py-0.5 text-[9px] font-black bg-[#8E1537]/15 text-[#8E1537] border border-[#8E1537]/40 font-mono uppercase flex items-center gap-0.5">
                          <Star className="w-2 h-2" fill="#8E1537" /> TOP
                        </span>
                      )}
                ...
                <div className="flex flex-wrap gap-1.5 mt-3 pt-3 border-t border-[#1F1F1F]">
                  <Button
                    ...
                    className="text-xs hover:bg-[#8E1537]/10 hover:text-[#8E1537] font-mono uppercase px-2"
                  >
                    <Pencil className="w-3.5 h-3.5 mr-1" />
                    Изменить
                  </Button>
                  <Button size="sm" variant="ghost" onClick={() => duplicateProduct(p.id)} className="text-xs hover:bg-[#8E1537]/10 hover:text-[#8E1537] font-mono uppercase px-2">
                  ...
                  {p.status === "archived" || p.status === "sold" || p.status === "reserved" ? (
                    <Button
                      ...
                      className="text-xs text-[#8E1537] hover:bg-[#8E1537]/10 font-mono uppercase px-2"
                    >
                  ...
                  <Button
                    ...
                    className="text-[#8E1537] hover:text-[#8E1537] hover:bg-[#8E1537]/10 ml-auto"
                  >
        ...
        <AlertDialogContent className="bg-[#0E0E0E] border-2 border-[#8E1537]">
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction
              onClick={onDelete}
              disabled={deleting}
              className="bg-[#8E1537] hover:bg-[#8E1537]/80 text-white font-mono uppercase"
            >
          ...
          <div className="space-y-3">
                  ...
                  .map((p) => {
                    ...
                    return (
                      <div key={p.id} className="flex justify-between py-0.5">
                        ...
                        <span className="text-[#8E1537] ml-2">→ {newPrice} ₽</span>
                      </div>
                    );
                  })}
                {selected.size > 3 && (
            ...
            <div className="flex gap-2 pt-2">
              <Button
                ...
                className="flex-1 bg-[#8E1537] text-black hover:bg-[#8E1537] hover:text-white font-black uppercase font-mono"
              >
...
function ProductForm({
  ...
  return (
    <Dialog open={open} onOpenChange={(o) => !o && onClose()}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto bg-[#0E0E0E] border-2 border-[#8E1537]">
        <DialogHeader>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 py-2">
          <div className="md:col-span-2">
            <Label className="text-[10px] uppercase tracking-widest font-mono text-[#8E1537]">{"// НАЗВАНИЕ *"}</Label>
            <Input
          ...
          <div>
            <Label className="text-[10px] uppercase tracking-widest font-mono text-[#8E1537]">{"// КАТЕГОРИЯ *"}</Label>
            <Select
              value={String(form.categoryId || "")}
              onValueChange={(v) => set("categoryId", v)}
            >
              <SelectTrigger className="mt-1 bg-[#0A0A0A] border-2 border-[#2A2A2A] focus:border-[#8E1537] font-mono">
          ...
          <div>
            <Label className="text-[10px] uppercase tracking-widest font-mono text-[#8E1537]">{"// СТАТУС"}</Label>
            <Select
          ...
          <div className="md:col-span-2">
            <Label className="text-[10px] uppercase tracking-widest font-mono text-[#8E1537]">{"// ОПИСАНИЕ"}</Label>
            <Textarea
              value={String(form.description || "")}
              onChange={(e) => set("description", e.target.value)}
              rows={3}
              className="mt-1 bg-[#0A0A0A] border-2 border-[#2A2A2A] focus:border-[#8E1537] font-mono text-sm"
              placeholder="Подробное описание..."
            />
          </div>

          <div>
            <Label className="text-[10px] uppercase tracking-widest font-mono text-[#8E1537]">{"// ЦЕНА (₽) *"}</Label>
            <Input
              type="number"
          ...
          <div>
            <Label className="text-[10px] uppercase tracking-widest font-mono text-[#8E1537]">{"// СТАРАЯ ЦЕНА"}</Label>
            <Input
              type="number"
          ...
          <div>
            <Label className="text-[10px] uppercase tracking-widest font-mono text-[#8E1537]">{"// ПОДПИСЧИКИ"}</Label>
            <Input
          ...
          <div>
            <Label className="text-[10px] uppercase tracking-widest font-mono text-[#8E1537]">{"// БЕЙДЖИ"}</Label>
            <Input
          ...
          <div>
            <Label className="text-[10px] uppercase tracking-widest font-mono text-[#8E1537]">{"// JSON МЕТА"}</Label>
            <Input
              value={String(form.metadata || "")}
              onChange={(e) => set("metadata", e.target.value)}
              className="mt-1 bg-[#0A0A0A] border-2 border-[#2A2A2A] focus:border-[#8E1537] font-mono text-xs"
              placeholder='{"country":"RU","age":"6 мес"}'
            />
          </div>

          <div className="md:col-span-2 pt-3 border-t border-[#1F1F1F]">
            <Label className="text-[10px] uppercase tracking-widest font-mono text-[#8E1537]">{"// ДАННЫЕ ДЛЯ ВЫДАЧИ ПОКУПАТЕЛЮ"}</Label>
          </div>

          <div>
            <Label className="text-[10px] uppercase tracking-widest font-mono text-[#8E1537]">Гарантия, дней</Label>
          ...
          <div>
            <Label className="text-[10px] uppercase tracking-widest font-mono text-[#8E1537]">Последняя проверка</Label>
          ...
          <div className="md:col-span-2">
            <Label className="text-[10px] uppercase tracking-widest font-mono text-[#8E1537]">Опубликовать по расписанию</Label>
          ...
          <div className="md:col-span-2">
            <Label className="text-[10px] uppercase tracking-widest font-mono text-[#8E1537]">Внутренняя заметка (не видна покупателю)</Label>
          ...
          <div>
            <Label className="text-[10px] uppercase tracking-widest font-mono text-[#8E1537]">{"// ЛОГИН *"}</Label>
            <Input
          ...
          <div>
            <Label className="text-[10px] uppercase tracking-widest font-mono text-[#8E1537]">{"// ПАРОЛЬ *"}</Label>
            <Input
          ...
          <div className="md:col-span-2">
            <Label className="text-[10px] uppercase tracking-widest font-mono text-[#8E1537]">{"// ИНСТРУКЦИЯ ПО ДОСТАВКЕ"}</Label>
            <Textarea
          ...
          <div className="md:col-span-2 flex items-center gap-3 pt-2">
            ...
            >
              <Star className="w-4 h-4" fill={form.featured ? "#8E1537" : "none"} />
        ...
        <div className="flex gap-3 pt-4">
          ...
          <Button
            onClick={save}
            disabled={saving}
            className="flex-1 bg-[#8E1537] text-black hover:bg-[#8E1537] hover:text-white font-black uppercase border-2 border-[#8E1537] hover:border-[#8E1537] font-mono tracking-wide"
          >
