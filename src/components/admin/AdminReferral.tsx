  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-4 mb-6">
        <MetricCard icon={Users} label="РЕФЕРАЛОВ" value={String(stats.totalReferrals)} color="#8E1537" />
        <MetricCard icon={MousePointerClick} label="КЛИКОВ" value={String(stats.totalClicks)} color="#8E1537" />
        <MetricCard icon={ShoppingCart} label="ЗАКАЗОВ" value={String(stats.totalOrders)} color="#8E1537" />
        <MetricCard icon={DollarSign} label="ЗАРАБОТАНО" value={formatPrice(stats.totalEarnings)} color="#8E1537" />
      </div>
      ...
      >
        <h3 className="font-black mb-4 flex items-center gap-2 uppercase tracking-tight">
          ...
          <span className="text-[#8E1537] font-mono text-xs">{"// КЛИКИ_ЗА_14_ДНЕЙ"}</span>
        </h3>
      ...
      >
        <h3 className="font-black mb-4 flex items-center gap-2 uppercase tracking-tight">
          ...
          <span className="text-[#8E1537] font-mono text-xs">{"// ТОП_РЕФЕРАЛОВ"}</span>
        </h3>
            ...
            {stats.topReferrers.map((r, i) => (
              <div key={r.code} className="flex items-center gap-3 py-2 border-b border-[#1F1F1F] last:border-0">
                ...
                <div className="flex items-center gap-4 text-xs font-mono">
                  <span className="text-[#8E1537]">{r.clicks} кликов</span>
                  <span className="text-[#8E1537]">{r.orders} зак.</span>
                  <span className="text-[#8E1537] font-black">{formatPrice(r.earnings)}</span>
                </div>
              </div>
            ))}
          </div>
