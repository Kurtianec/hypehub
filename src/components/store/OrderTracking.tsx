...
export function OrderTracking({ id }: { id: string }) {
  ...
  return <main className="flex-1 pb-16 pt-28 md:pt-32">
    <div className="mx-auto w-full max-w-3xl px-4">
      ...
      <section className="order-panel rounded-2xl border bg-card p-4 shadow-sm md:p-6">
        ...
        <div className="space-y-1">
          {order.events.map((event, index) => <div key={event.id} className="relative flex gap-3 pb-5 last:pb-0">
            ...
            <div className="min-w-0"><div className="break-words text-sm font-semibold">{event.label}</div><div className="mt-0.5 text-xs text-muted-foreground">{new Date(event.createdAt).toLocaleString("ru-RU")} · {event.actor}</div></div>
          </div>)}
        </div>
      ...
      <div className="mt-4 flex flex-wrap gap-2"><Link href="/account"><Button variant="outline">Все заказы</Button></Link><Link href="/"><Button variant="ghost">В каталог</Button></Link></div>
    </div>
  </main>;
}
