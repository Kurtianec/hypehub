...
export async function POST(req: NextRequest) {
  ...
  const response = await fetch("https://api.resend.com/emails", { method: "POST", headers: { Authorization: `Bearer ${process.env.RESEND_API_KEY}`, "Content-Type": "application/json" }, body: JSON.stringify({ from: process.env.EMAIL_FROM || "HypeHub <onboarding@resend.dev>", to: [email], subject: "Код входа в HypeHub", html: `<div style="font-family:Arial;background:#0a0a0a;color:#fff;padding:32px"><h2>Вход в HypeHub</h2><p>Ваш одноразовый код:</p><div style="font-size:32px;font-weight:800;letter-spacing:8px;color:#F7A600">${code}</div><p style="color:#999">Код действует 10 минут. Никому его не сообщайте.</p></div>` }) });
