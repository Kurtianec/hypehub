      ...
      <div className="container mx-auto px-4 max-w-4xl">
        ...
        >
          <div className="text-center">
            <div className="text-5xl font-black text-[#8E1537] font-mono">{avgRating}</div>
            <div className="flex gap-0.5 justify-center mt-1">
              {[1, 2, 3, 4, 5].map((s) => (
                <Star
                  key={s}
                  className="w-4 h-4"
                  fill={s <= Math.round(parseFloat(avgRating)) ? "#8E1537" : "none"}
                  stroke="#8E1537"
                />
              ))}
            </div>
        ...
        {showForm && (
          <motion.div
            ...
            className="bg-[#0E0E0E] border-2 border-[#8E1537] p-6 mb-8 overflow-hidden"
            style={{ clipPath: "polygon(0 0, calc(100% - 16px) 0, 100% 16px, 100% 100%, 16px 100%, 0 calc(100% - 16px))" }}
          >
            ...
            <div className="space-y-4">
              ...
              <div>
                <Label className="text-[10px] uppercase tracking-widest font-mono text-[#8E1537]">{"// ОЦЕНКА"}</Label>
                <div className="flex gap-2 mt-1.5">
                  {[1, 2, 3, 4, 5].map((s) => (
                    <button
                      ...
                      style={{
                        borderColor: s <= rating ? "#8E1537" : "#2A2A2A",
                        background: s <= rating ? "#8E153726" : "transparent",
                      }}
                      aria-label={`${s} звёзд`}
                    >
                      <Star
                        className="w-5 h-5"
                        fill={s <= rating ? "#8E1537" : "none"}
                        stroke="#8E1537"
                      />
                    </button>
                  ))}
              ...
              <div>
                <Label className="text-[10px] uppercase tracking-widest font-mono text-[#8E1537]">{"// КАКОЙ ТОВАР КУПИЛИ (НЕОБЯЗАТЕЛЬНО)"}</Label>
                <Input
                  value={product}
              ...
              <div>
                <Label className="text-[10px] uppercase tracking-widest font-mono text-[#8E1537]">{"// ОТЗЫВ"}</Label>
                <textarea
                  value={text}
        ...
        {/* Submitted success */}
        {submitted && (
          <div className="bg-[#0E0E0E] border-2 border-[#8E1537] p-4 mb-6 flex items-center gap-3"
            style={{ clipPath: "polygon(0 0, calc(100% - 12px) 0, 100% 12px, 100% 100%, 12px 100%, 0 calc(100% - 12px))" }}
          >
            <CheckCircle2 className="w-5 h-5 text-[#8E1537]" strokeWidth={2.5} />
            <div className="text-sm font-mono">
              <span className="text-[#8E1537] font-bold">{"// THANK_YOU"}</span>
              ...
              >
                <div className="flex items-start justify-between mb-3">
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 bg-[#1A1A1A] border-2 border-[#8E1537]/40 flex items-center justify-center font-black text-[#8E1537] font-mono">
