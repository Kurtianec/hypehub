    ...
    <button
      ...
      className={`absolute bottom-2 right-2 w-8 h-8 flex items-center justify-center border-2 transition-all ${
        isActive
          ...
          : "bg-black/80 backdrop-blur-sm text-[#8E1537] border-[#8E1537]/40 hover:border-[#8E1537]"
      }`}
...
export function CompareBar({
  ...
  return (
    ...
    >
      <div
        className="bg-[#0E0E0E] border-2 border-[#8E1537] p-3 flex items-center gap-3"
        style={{ clipPath: "polygon(0 0, calc(100% - 14px) 0, 100% 14px, 100% 100%, 14px 100%, 0 calc(100% - 14px))" }}
      >
        <div className="flex items-center gap-2 text-xs font-mono uppercase text-[#8E1537] flex-shrink-0">
          <GitCompare className="w-4 h-4" strokeWidth={2.5} />
          <span className="hidden md:inline">СРАВНЕНИЕ:</span>
          <span className="bg-[#8E1537] text-black px-1.5 py-0.5 font-black">{products.length}</span>
        </div>
          ...
          {products.map((p) => (
            <div key={p.id} className="flex-shrink-0 bg-[#1A1A1A] border border-[#2A2A2A] px-2 py-1 text-[10px] font-mono uppercase flex items-center gap-1">
              ...
              <button
                ...
                className="text-[#8E1537] hover:text-[#8E1537]/70"
                aria-label="Убрать"
              >
        ...
        <Button
          ...
          className="bg-[#8E1537] text-black hover:bg-[#8E1537] font-black uppercase border-2 border-[#8E1537] hover:border-[#8E1537] font-mono text-xs flex-shrink-0"
        >
          Сравнить
        </Button>
        <button
          onClick={onClear}
          className="text-[#888] hover:text-[#8E1537] flex-shrink-0 p-1"
          aria-label="Очистить"
        >
          <X className="w-4 h-4" />
        </button>
...
export function CompareModal({
  ...
  const rows: { label: string; render: (p: Product) => React.ReactNode }[] = [
    ...
    { label: "Цена", render: (p) => <span className="font-black text-[#8E1537]">{formatPrice(p.price, p.currency)}</span> },
    ...
    { label: "Скидка", render: (p) => {
      ...
      return <span className="text-[#8E1537] font-bold">−{d}%</span>;
    }},
    { label: "Страна", render: (p) => (getMeta(p).country as string) || "—" },
    { label: "Возраст", render: (p) => (getMeta(p).age as string) || "—" },
    { label: "Монетизация", render: (p) => getMeta(p).monetization ? <Check className="w-4 h-4 text-[#8E1537]" strokeWidth={3} /> : <Minus className="w-4 h-4 text-[#888]" /> },
    { label: "Верификация", render: (p) => getMeta(p).verified ? <Check className="w-4 h-4 text-[#8E1537]" strokeWidth={3} /> : <Minus className="w-4 h-4 text-[#888]" /> },
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <motion.div
        ...
        className="relative bg-[#0E0E0E] border-2 border-[#8E1537] w-full max-w-4xl max-h-[85vh] overflow-y-auto"
      ...
      >
        <div className="sticky top-0 bg-[#0E0E0E] border-b-2 border-[#1F1F1F] p-4 flex items-center justify-between z-10">
          <div className="flex items-center gap-3">
            <GitCompare className="w-5 h-5 text-[#8E1537]" strokeWidth={2.5} />
