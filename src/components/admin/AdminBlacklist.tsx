    ...
    <div>
      <div className="flex flex-col sm:flex-row justify-between gap-3 mb-6">
        ...
        <Button
          ...
          className="bg-[#8E1537] hover:bg-[#8E1537]/80 text-white font-black uppercase border-2 border-[#8E1537] font-mono tracking-wide"
        >
      ...
      {/* Warning banner */}
      <div
        className="mb-6 bg-[#8E1537]/10 border-2 border-[#8E1537]/40 p-4 flex items-start gap-3"
        style={{ clipPath: "polygon(0 0, calc(100% - 12px) 0, 100% 12px, 100% 100%, 12px 100%, 0 calc(100% - 12px))" }}
      >
        <AlertTriangle className="w-5 h-5 text-[#8E1537] flex-shrink-0 mt-0.5" strokeWidth={2.5} />
        <div className="text-xs font-mono">
          <div className="font-black uppercase text-[#8E1537] mb-1">// ВНИМАНИЕ</div>
      ...
      <Dialog open={adding} onOpenChange={(o) => !o && setAdding(false)}>
        <DialogContent className="max-w-md glass-strong border-white/10">
          ...
          <div className="space-y-3">
            ...
            <div className="bg-[#8E1537]/10 border border-[#8E1537]/30 p-2 text-[10px] font-mono text-[#888]">
              &gt; Будет заблокировано: оформление заказов, обращения в поддержку с этого источника
            </div>
            <div className="flex gap-2 pt-2">
              <Button
                onClick={addEntry}
                className="flex-1 bg-[#8E1537] hover:bg-[#8E1537]/80 text-white font-black uppercase font-mono"
              >
                Заблокировать
              </Button>
        ...
        {entries.map((e) => {
          ...
          return (
            <motion.div
              ...
              className="flex items-center gap-3 py-2 px-3 bg-[#0A0A0A] border border-[#1F1F1F] hover:border-[#8E1537]/50 transition-colors"
            >
              ...
              <button
                onClick={() => onDelete(e)}
                className="text-[#888] hover:text-[#8E1537] transition-colors px-2 py-1 text-[10px] font-mono uppercase border border-[#2A2A2A] hover:border-[#8E1537]"
              >
